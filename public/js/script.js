'use strict'
import { showTime } from './time.js'; //время, дата, приветствие
import { getWeather } from './wether.js'; //погоды
import { openBox, state } from './option.js'; //настройки
import { getSlidePrev, getSlideNext, seachImg } from './bg.js'; //фон
import { openTodoBox } from './todo.js'; //список дел
import { setLocalStorage, getLocalStorage } from './save.js'; //сохранение данных инпутов в local storage
import { getQuotes } from './quotes.js'; //цитаты
import { playAudio, playNext, playPrev, render, changeTrack, clickTimeline, volumeClick, clickVolumeline } from './player.js'; //плеер
import { instalLang } from './lang.js'; //язык приложения
...
window.addEventListener('load', () => { instalLang(state.language) }); //установка языка приложения (строки текста)
window.addEventListener('load', showTime); //запускает функцию обновления время, дата, приветствие

//настройки ---------------------------------------------------------------------------------------------
const optionBTN = document.querySelector('.optionBTN'); //кнопка настроек
optionBTN.addEventListener('click', openBox); //открытие окна настроек
//-------------------------------------------------------------------------------------------------------

//слайдер картинок фона ---------------------------------------------------------------------------------
window.addEventListener('load', seachImg); //загрузка фоновго изображения
const leftArrow = document.querySelector(".leftArrow"); //стрелка влевоs
leftArrow.addEventListener("click", getSlidePrev); //нажатие на левую стрелку
const rightArrow = document.querySelector(".rightArrow"); //стрелка вправо
rightArrow.addEventListener("click", getSlideNext); //нажатие на правую стрелку
const searchBTN = document.querySelector('.optionBox__searchBTN'); //кнопка поиска картинок
searchBTN.addEventListener('click', () => { seachImg(); openBox(); }); //нажатие на кнопку поиска картинок
const searchBG = document.querySelector('.optionBox__searchBG'); //строка поиска картинок
searchBG.addEventListener('keypress', (e) => { if (e.key === 'Enter') {seachImg(); openBox();} }); //нажатие ввод в строрке поиска
//--------------------------------------------------------------------------------------------------------

//блок погода --------------------------------------------------------------------------------------------
window.addEventListener('load', () => { getWeather(state.language) }); //обновление погоды при загрузке страницы
const city = document.querySelector('.wether__city'); //инпут (город)
city.addEventListener('change', () => { getWeather(state.language) }); //обновление погоды при вводе города
//--------------------------------------------------------------------------------------------------------

//блок цитат ---------------------------------------------------------------------------------------------
window.addEventListener('load', () => { getQuotes(state.language) }); //обновление цитаты при загрузке страницы
const qUpdate = document.querySelector('.quote__update'); //кнопка обновления цитаты
qUpdate.addEventListener('click', () => { getQuotes(state.language) }); //обновление цитаты по нажатию на кнопку
//--------------------------------------------------------------------------------------------------------

//плеер --------------------------------------------------------------------------------------------------
window.addEventListener('load', render); //запуск общего рендера (список треков, время проигрывания и ТД)
const prev = document.querySelector('.player__play-prev'); //кнопка предыдущий трек
const play = document.querySelector('.player__play'); //кнопка плей/пауза
const next = document.querySelector('.player__play-next');//кнопка следующий трек
play.addEventListener('click', playAudio); //запуск плеера
prev.addEventListener('click', playPrev); //предыдущий трек
next.addEventListener('click', playNext); //следующий трек
const playerList = document.querySelector('.player__list'); //блок списка треков
playerList.addEventListener('click', e => { changeTrack(e) }); //нажатие на трек в списке
const duration = document.querySelector('.player__duration'); //строка заполнения времени
duration.addEventListener("click", e => { clickTimeline(e) }); //нажатие по временной шкале
const volumeBTN = document.querySelector('.player__volume'); //кнопка громкости
volumeBTN.addEventListener('click', volumeClick); //нажатие по кнопке громкости
const volumeLevel = document.querySelector('.player__volumeLevel'); //полоса громкости
volumeLevel.addEventListener("click", e => { clickVolumeline(e) }); //нажатие по полосе громкости
//-------------------------------------------------------------------------------------------------------

//список дел ---------------------------------------------------------------------------------------------
const todoBTN = document.querySelector('.todoBTN'); //кнопка тодо
todoBTN.addEventListener('click', openTodoBox); //открытие окна список дел
//-------------------------------------------------------------------------------------------------------
